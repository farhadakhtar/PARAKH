"""
PARAKHfrom pathlib import Path
...
def build_clean_record_from_validated(
    ...
    return CleanRecord(
        ...
        sanction_amount=clean["sanction_amount"],
        amount_released=clean["amount_released"],
        amount_utilized=clean["amount_utilized"],
        date_sanction=clean["date_sanction"],
        date_start=clean["date_start"],
        date_completion=clean["date_completion"],
        work_category=clean["work_category"],
    )
...
def pipeline(
    ...
    validation_summary = {
        ...
        "errors": all_errors[:20],  # first 20 for summary; full list available via corpus
